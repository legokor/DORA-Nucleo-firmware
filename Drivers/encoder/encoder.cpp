...
void Encoder::init(TIM_HandleTypeDef* timer,
    ...
    // 0.015s needs to pass without encoder reading to declare the motor as stopped
    this->maxOverflowCount = (float) timerFrequency / timerPeriod * 0.015f + 1;
    this->stopped = true;
    ...
    HAL_TIM_IC_Start_IT(timer, captureChannelA);
...
void Encoder::handleInputCapture(TIM_HandleTypeDef* htim) {
    ...
    int32_t dir = (A != B) ^ reversed ? +1 : -1;
    absolutePosition += dir;
