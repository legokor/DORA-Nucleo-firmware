...
void Lcd::init(I2C_HandleTypeDef* i2c, IRQn_Type i2cIr, uint8_t address, uint16_t commandBufferSize) {
	this->i2c = i2c;
	this->address = address;
	this->commandBufferSize = commandBufferSize / sizeof(uint32_t);
	this->commandBufferReadPtr = 0;
	this->commandBufferWritePtr = 0;
	this->commandBuffer = (uint32_t*)malloc(commandBufferSize / sizeof(uint32_t) * sizeof(uint32_t));
	this->i2cIr = i2cIr;
	this->backlight = 0;
...
void Lcd::send(uint32_t data){
	commandBuffer[commandBufferWritePtr] = data;
	HAL_NVIC_DisableIRQ(i2cIr);
	enablePeriodHandle = false;
	if(commandBufferWritePtr == commandBufferReadPtr && !transmissionInProgress){
		commandBufferWritePtr++;
		if(commandBufferWritePtr == commandBufferSize)
			commandBufferWritePtr = 0;
...
void Lcd::handleTransmitCplt(I2C_HandleTypeDef* i2c){
	if(this->i2c != i2c)
		return;
	if(commandBufferWritePtr == commandBufferReadPtr){
		transmissionInProgress = false;
		return;
	}
	if(periodCounter < periodCount)
		return;
	HAL_I2C_Master_Transmit_IT(i2c, address, (uint8_t *)(&commandBuffer[commandBufferReadPtr]), 4);
	if(commandBuffer[commandBufferReadPtr] & 0x01){
		commandBufferReadPtr++;
		if(commandBufferReadPtr == commandBufferSize)
			commandBufferReadPtr = 0;
	}else{
		periodCounter = 0;
	}
}
