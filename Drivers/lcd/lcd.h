...
public:
	 ...
	 * Initializes the LCD
	 */
	void init(I2C_HandleTypeDef* i2c, IRQn_Type i2cIr, uint8_t address, uint16_t commandBufferSize);
...
private:
	...
	volatile uint16_t commandBufferSize;
	volatile uint16_t commandBufferWritePtr;
	volatile uint16_t commandBufferReadPtr;
	volatile uint32_t* commandBuffer;
