...
private:
	...
	const uint8_t frameStartFlag;
	const uint8_t frameEndFlag;
	const uint8_t escapeFlag;
...
public:
	BinaryUartImpl(UART_HandleTypeDef* huart, IRQn_Type txDmaIr,
			uint8_t frameStartFlag, uint8_t frameEndFlag, uint8_t escapeFlag)
					: huart(huart), txDmaIr(txDmaIr), frameStartFlag(frameStartFlag),
					  frameEndFlag(frameEndFlag), escapeFlag(escapeFlag)
	{
			...
			if(rxIsFrameOpened)
			{
				while(readNext(newC) && !restart)
				{
					...
					else if(rxEscapeFlagActive)
					{
						...
						{
						case 1:
							rxFrameBuffer[rxOpenFrameSize++] = frameStartFlag;
							break;
						case 2:
							rxFrameBuffer[rxOpenFrameSize++] = escapeFlag;
							break;
						case 3:
							rxFrameBuffer[rxOpenFrameSize++] = frameEndFlag;
							break;
						default:
							rxIsFrameOpened = false;
							status |= status::RX_ESCAPE_ERROR;
